...
class Action(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='action')
...
class Tweet(models.Model):
    ...
    def hit_page(self, user, comment=False):
        # increase in num of hits for user who vistited the posts, and for post that has visited
        if comment==False: 
            self.hits += 1
            user.profile.num_of_hits += 1
        else: # ingoring increase in num of hits during releoding page after comment
            self.hits -= 1
            user.profile.num_of_hits -= 1
        self.save()
        user.profile.save()
